...
def answer_message(request):
    ...
    if request.method == "POST":
        ...
        if answer_form.is_valid():
            ...
            #latest_message = Message.objects.last(latest_message.sender!=request.user)
            latest_message = Message.objects.exclude(sender=request.user).order_by('-time').last()
            if latest_message is not None:
                username_sender = latest_message.sender.username
            else:
                message = "check the data"
                answer.receiver = latest_message.sender
            answer.save()
            message = "answer sent"
            return redirect('get_my_message')
